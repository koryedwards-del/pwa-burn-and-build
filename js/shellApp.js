...
import { QUESTIONNAIRE_WELCOME_URL, CREATOR_HOST_ORIGIN, MARKETING_ORIGIN } from './siteUrls.js';

const BNB_WEBSITE_URL = `${MARKETING_ORIGIN}/?browse=1`;
const BNB_CREATOR_URL = QUESTIONNAIRE_WELCOME_URL;
  ...
  return `
    <div class="screen">
      ...
      <div class="import-block">
        ...
        <p class="import-note">Need a new program? <a href="../questionnaire/#welcome">Create one on the website →</a></p>
      </div>
    </div>`;
}
        ...
        <div class="meal-card${complete ? ' meal-complete' : ''}${expanded ? ' meal-expanded' : ''}">
          <button type="button" class="meal-card-header" data-toggle="${slot.label}">
            <div>
              ...
              ${!expanded && !logged.length && progress.required ? '<div class="meal-empty-hint">Choose your food.</div>' : ''}
...
function bindEvents() {
  ...
  document.querySelector('[data-toggle-coach-banner]')?.addEventListener('click', () => {
    ...
    if (opening && showCoachBanner()) markCoachDayComplete(currentProgramDay());
    render();
  });

  if (store.screen === 'coach') bindCoachScreen();
